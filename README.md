...
| Ctrl+L | Clear the terminal buffer (Cmd+L on macOS) |
| Ctrl+Shift+D | Duplicate the current SSH terminal tab |
...
- SFTP downloads are confined to the chosen destination folder, with
  path-traversal and symlink checks to prevent writes outside it.
- Server Stats and the other tools connect using the same host-key policy and
  credential handling as the rest of the application.
...
5. Select Test Connection to validate the connection details.
